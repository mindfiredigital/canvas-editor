...
class RangeManager {
  ...
  togglePendingStyle(key) {
    ...
    this.pendingStyle = __spreadProps(__spreadValues({}, this.pendingStyle || {}), {
      [key]: !current
    });
    this.setRangeStyle();
  }
...
class HyperlinkParticle {
  constructor(draw) {
    ...
    __publicField(this, "hyperlinkPreviewDom");
    this.draw = draw;
    this.options = draw.getOptions();
    this.container = draw.getContainer();
...
class Draw {
  ...
  computeRowList(innerWidth, elementList) {
    ...
    for (let i = 0; i < elementList.length; i++) {
      ...
      const ascent = element.imgDisplay !== ImageDisplay.INLINE && element.type === ElementType.IMAGE || element.type === ElementType.LATEX ? metrics.height + rowMargin + paragraphSpacingBefore : metrics.boundingBoxAscent + rowMargin + paragraphSpacingBefore;
...
class CommandAdapt {
  ...
  paragraphSpacing(payload) {
    ...
    const positionList = this.position.getPositionList();
    const elementList = this.draw.getElementList();
    const touchedZeroIndexes = new Set();
    for (let p = 0; p < positionList.length; p++) {
      const position = positionList[p];
      const rowSet = rangeRow.get(position.pageNo);
      if (!rowSet)
