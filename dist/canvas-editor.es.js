    ...
    if (el.type === ElementType.TITLE) {
      ...
      if (el.trList) {
        for (let t = 0; t < el.trList.length; t++) {
          ...
          const trId = getUUID();
          tr.id = trId;
          if (tr.minHeight === void 0 || tr.minHeight === null) {
...
class CursorAgent {
  constructor(draw, canvasEvent) {
    ...
    agentCursorDom.oninput = (evt) => this._input(evt);
...
class RangeManager {
  ...
  togglePendingStyle(key) {
    ...
    const current = this.pendingStyle && key in this.pendingStyle ? this.pendingStyle[key] : anchor == null ? void 0 : anchor[key];
    this.pendingStyle = {
      ...this.pendingStyle || {},
      [key]: !current
    };
  ...
  getContentStyles() {
    ...
    if (this.pendingStyle && startIndex === endIndex) {
      if (this.pendingStyle.bold !== void 0)
        bold = !!this.pendingStyle.bold;
      if (this.pendingStyle.italic !== void 0)
        italic = !!this.pendingStyle.italic;
      if (this.pendingStyle.underline !== void 0)
        underline = !!this.pendingStyle.underline;
      if (this.pendingStyle.strikeout !== void 0)
...
class TableParticle {
  ...
  _drawBorder(ctx, element, startX, startY) {
    ...
    for (let t = 0; t < trList.length; t++) {
      ...
      for (let d = 0; d < tr.tdList.length; d++) {
        ...
        const trNext = (_c = trList[t + 1]) == null ? void 0 : _c.tdList[d];
  ...
  _drawPageBreakSeparator(ctx, y, color) {
    const margins = this.draw.getMargins();
...
class HyperlinkParticle {
  constructor(draw) {
    ...
    this.options = draw.getOptions();
    this.container = draw.getContainer();
    const { hyperlinkPopupContainer, hyperlinkDom, hyperlinkPreviewDom } = this._createHyperlinkPopupDom();
...
PageBreakParticle.font = "Yahei";
PageBreakParticle.fontSize = 12;
PageBreakParticle.displayName = "\u5206\u9875\u7B26";
PageBreakParticle.lineDash = [3, 1];
class Watermark {
  constructor(draw) {
    this.draw = draw;
    this.options = draw.getOptions();
...
const encodedJs$1 = "";
...
const encodedJs = "";
...
class Draw {
  ...
  computeRowList(innerWidth, elementList) {
    ...
    for (let i = 0; i < elementList.length; i++) {
      ...
      if (element.type === ElementType.IMAGE || element.type === ElementType.LATEX) {
        ...
        const firstTrScaledHeight = (((_c = trList[0]) == null ? void 0 : _c.height) || 0) * scale;
        const tableStartsOnNewPage = accumulatedPageHeight + tableRowMarginHeight + firstTrScaledHeight > pageHeight;
        ...
        if (tableExceedsPage && isTopLevel) {
          ...
          if (deleteCount) {
            ...
            const availableScaledHeight = Math.max(0, pageHeight - accumulatedPageHeight - tableRowMarginHeight - accumulatedTrHeight);
            const [currentPageTdList, nextPageTdList] = this.splitTrListByPageHeight(overflowTr.tdList, availableScaledHeight, scale);
            const hasOverflow = nextPageTdList.some((td) => {
              var _a2;
              return (((_a2 = td.rowList) == null ? void 0 : _a2.length) || 0) > 0;
            });
            const onlyOneTrSplitting = cloneTrList.length === 1 && !hasOverflow;
            if (onlyOneTrSplitting) {
              trList.push(overflowTr);
              const newTableHeight = trList.reduce((pre, cur) => pre + cur.height, 0);
              element.height = newTableHeight;
              metrics.height = newTableHeight * scale;
              metrics.boundingBoxDescent = metrics.height;
              this.tableParticle.computeRowColInfo(element);
            } else if (currentPageTdList.every((td) => {
              var _a2;
              return !((_a2 = td.rowList) == null ? void 0 : _a2.length);
            })) {
              if (trList.length === 0) {
                trList.splice(deleteStart, 0, ...cloneTrList);
                const restoredTableHeight = trList.reduce((pre, cur) => pre + cur.height, 0);
                ...
                cloneElement.height = cloneTrList.reduce((pre, cur) => pre + cur.height, 0);
              ...
              const overflowTrHeight = Math.max(overflowTr.minHeight || 0, ...nextPageTdList.map((td) => td.height || 0));
              const splitTrHeight = Math.max(overflowTr.minHeight || 0, ...currentPageTdList.map((td) => td.height || 0));
    ...
    for (let i = 0; i < rowList.length; i++) {
      ...
      for (let j = 0; j < curRow.elementList.length; j++) {
        ...
        if (element.underline) {
          const lineSpacing = element.rowMargin || defaultRowMargin;
          const elSizePx = (element.size || this.options.defaultSize) * PX_PER_PT * scale;
  ...
  render(payload) {
    ...
    if (postComputeRange.startIndex >= 0 && postComputeRange.startIndex === postComputeRange.endIndex) {
  ...
  mergeTableFragments(elementList, skipStateMutation = false) {
    return this._mergeTableFragments(elementList, skipStateMutation);
  }
  _mergeTableFragments(elementList, skipStateMutation = false) {
    var _a, _b, _c, _d;
    let i = 0;
    while (i < elementList.length - 1) {
      const cur = elementList[i];
      const next = elementList[i + 1];
      const isMergeable = cur.type === ElementType.TABLE && next.type === ElementType.TABLE && !!cur.id && cur.id === next.id && !!((_a = cur.trList) == null ? void 0 : _a.length) && !!((_b = next.trList) == null ? void 0 : _b.length);
      if (!isMergeable) {
        i++;
        continue;
      }
      ...
      const isFragmentPair = !!lastTr.originalRowId && lastTr.originalRowId === firstTr.originalRowId;
      const positionContext = this.position.getPositionContext();
      const range = this.range.getRange();
      const cursorTdValueOffsets = {};
      if (isFragmentPair && sameColCount) {
        for (let t = 0; t < firstTr.tdList.length; t++) {
          ...
          const skipFirstZero = lastValue.length > 0 && ((_c = firstValue[0]) == null ? void 0 : _c.value) === ZERO;
          const offset = lastValue.length - (skipFirstZero ? 1 : 0);
          firstTd.value = [
            ...lastValue,
            ...firstValue.slice(skipFirstZero ? 1 : 0)
          ];
          firstTd.rowList = void 0;
          firstTd.positionList = void 0;
      ...
      if (!skipStateMutation && positionContext.isTable && positionContext.index !== void 0) {
        if (positionContext.index === i + 1) {
          positionContext.index = i;
          const trIndex = (_d = positionContext.trIndex) != null ? _d : 0;
          if (isFragmentPair && sameColCount) {
            positionContext.trIndex = trIndex === 0 ? lastTrIndex : lastTrIndex + trIndex;
            if (trIndex === 0 && positionContext.tdIndex !== void 0 && cursorTdValueOffsets[positionContext.tdIndex] > 0 && range.startIndex >= 0) {
  ...
  splitTrListByPageHeight(tdList, availableScaledHeight, scale) {
    var _a, _b, _c;
    ...
    for (const td of tdList) {
      ...
      for (const row of td.rowList || []) {
        ...
        if (!hasOverflowed && accumulatedHeight + rowHeight <= availableScaledHeight) {
      ...
      const toUnscaledTdHeight = (rows) => {
        const scaledRowsHeight = rows.reduce((pre, cur) => pre + (cur.height || 0), 0);
        return scaledRowsHeight / scale + tdGap;
      };
      const emptyRowHeight = (((_b = (_a = td.rowList) == null ? void 0 : _a[0]) == null ? void 0 : _b.height) || 0) / scale + tdGap;
      currentTd.rowList = this.updateRowList(currentRowList);
      currentTd.value = this.rebuildValueFromRowList(currentTd.rowList);
      currentTd.height = currentRowList.length ? toUnscaledTdHeight(currentRowList) : emptyRowHeight;
      ...
      nextTd.height = overflowRowList.length ? toUnscaledTdHeight(overflowRowList) : emptyRowHeight;
  ...
  updateRowList(rowList) {
    ...
    return rowList.map((row) => {
      ...
      startIndex += ((_a = row.elementList) == null ? void 0 : _a.length) || 0;
      return updated;
    });
  }
  rebuildValueFromRowList(rowList) {
    return rowList.flatMap((row) => (row.elementList || []).map((element) => {
      const { metrics, style: style2, ...rest } = element;
      return rest;
    }));
  }
}
...
class CommandAdapt {
  ...
  font(payload) {
    ...
    const fonts = document == null ? void 0 : document.fonts;
    if (fonts == null ? void 0 : fonts.load) {
  ...
  paragraphSpacing(payload) {
    ...
    const positionList = this.position.getPositionList();
    const elementList = this.draw.getElementList();
    const touchedZeroIndexes = /* @__PURE__ */ new Set();
    for (let p = 0; p < positionList.length; p++) {
      const position = positionList[p];
      const rowSet = rangeRow.get(position.pageNo);
      if (!rowSet)
      ...
      if (!rowSet.has(position.rowNo))
        continue;
      let q = p;
      while (q >= 0 && elementList[q].value !== ZERO)
        q--;
      if (q >= 0)
        touchedZeroIndexes.add(q);
    }
    touchedZeroIndexes.forEach((idx) => {
      if (payload.before !== void 0) {
        elementList[idx].paragraphSpacingBefore = payload.before;
      }
      if (payload.after !== void 0) {
  ...
  tableRowSeparator(payload) {
    ...
    const originalElementList = this.draw.getOriginalElementList();
    const element = originalElementList[index2];
    const tr = (_a = element == null ? void 0 : element.trList) == null ? void 0 : _a[trIndex];
    if (!tr)
      return;
    ...
    tr.tdList.forEach((td) => {
      if (color !== void 0)
        td.borderBgBottom = color;
      if (width !== void 0)
        td.borderWidthBottom = width;
    });
    if (trNext) {
      trNext.tdList.forEach((td) => {
        if (color !== void 0)
          td.borderBgTop = color;
        if (width !== void 0)
          td.borderWidthTop = width;
      });
    }
    const { endIndex } = this.range.getRange();
    this.draw.render({
...
class DOMEventHandlers {
  ...
  static register(container, data2, options = {}) {
    ...
    DOMEventHandlers.instance = new Editor(container, data2, options);
